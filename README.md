...
- One term: **"`ME-IRL`"** = **Maximum Entropy** `IRL`. The probability distribution over trajectories is in the form `exp`(`-cost[features(traj), θ]`), to model that **agents are exponentially more likely to select trajectories with lower cost**.
- One Q&A: The trajectory object is first mapped to some **feature vector** (`speed`, `acceleration` ...). _How to then derive a `cost` (or `reward`) from these features?_
...
- About **_"feature-matching"_** `IRL`:
  - > "Our goal is to find a generative model that yields trajectories that are similar to the observations, where the **features** serve as a **measure of similarity**."
  - Among all the distributions that **match features**, they to select the one that **maximizes the entropy**.
- One quote about the _maximum likelihood approximation_ in `ME-IRL`:
  - To estimate the **expected feature values**, the authors compute the **feature values of the most likely trajectory**, instead of computing the expectations by **sampling**.
